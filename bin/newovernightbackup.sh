...
PROG=/home/matz/bin/newrolltar.sh
LOW=250000
...
#directory directoryOrFile  tarfilename
#************************************************************************
TABLE="
...
/home/matz       		  .emacs   			     dot.emacs \
/var/spool/mail            matz       		     spool_mail \
/home/matz/skule           research			     research   \
/home            		   hfe     			     hfemail"
#************************************************************************
...
do
	cddir=$1;	fnOrdir=$2;	tfn=$3; shift; shift
	echo $cddir/$fnOrdir to file $BACKUPS/$tfn.X.tarz
	if cd $cddir
	then
		verbmsg `pwd`
	else
	...
	if test ! -d $fnOrdir -a ! -f $fnOrdir
	then
		die "no file or directory $bdir"
	fi
done
echo '---------------------------------------------'
echo
...
do
	...
	################# doit ###############################
	if $PROG $fnOrDir $BACKUPS/$tfn
	then
		verbmsg '================================================='
		verbmsg  $0: $PROG $fnOrDir $BACKUPS/$tfn returns okay
#		verbmsg '================================================='
	else
		#is this right? Why not press on and attempt further backups?
		#what if /backup is full?
		die returns non-zero $PROG $fnOrDir $BACKUPS/$tfn
	fi		
done
