...
if test ! -d $1 -a ! -f $1
then
	echo "$1 not a directory or file. Exit with no backup"
	exit 2
fi
...
then
	...
	NEW_FILES=`find $1 -newer $latest -print`
	if test -z "$NEW_FILES"
	then
		verbmsg no new files, so:
		verbmsg check if filesystem is same as latest backup..
		verbmsg tar $TAR_DOPT $latest $1
		if tar $TAR_DOPT $latest $1 > /dev/null
		then
			echo '*****' NO TAR because no new files AND dir $1 same as `file $latest`
		...
		echo new files exist in $1..
		verbmsg find -newer $latest finds:
		verbmsg $NEW_FILES
	fi
fi
	
verbmsg "about to execute tar $TAR_COPT $tarfile $1"
if tar $TAR_COPT $tarfile $1
then
	ls -l $tarfile > /dev/null
else
	die returns failure: tar $TAR_COPT $tarfile $1
fi
