(defun xcodebuild-list-shell ()
  ...
  (interactive)
  (shell-command "/usr/bin/xcodebuild -help ; xcodebuild -list"))

(defun xcodebuild-list ()
  "Returns Xcode target/configuration lines for project in current directory."
  (interactive)
  (process-lines "/usr/bin/xcodebuild" "-project" "AIR.cocoatouch.xcodeproj" "-list"))
...
(defun xcodebuild-aot ()
  "build aot knowing magic xcode tokens"
  (interactive)
  (cd "~/Perforce/AOT-llvm3-branch_mzaleski_deskside/llvm3branch/code/products/AIR")
      (let*
		  ((tmp-file (make-temp-file "xcodebuild" nil ".log"))
		  (cmd (concat "time ( /usr/bin/xcodebuild -project AIR.cocoatouch.xcodeproj -arch \"iPhone 5.1 Simulator\" -target \"SDK with OSX AVMShell\" -configuration Debug"
                       " | tee " tmp-file
...
(defun xcodebuild (archs cfg tgt cmds)
  ...
  (interactive (progn
                 ...
                 (let ((xcode-lines (xcodebuild-list)))
                   (let ((archs '("x86_64" "i386" "iPhone 5.1 Simulator"))
  ...
  (let ((archs-arg (concat " ONLY_ACTIVE_ARCH=NO ARCHS=\"" archs "\""))
        ...
        (tmp-file (make-temp-file "xcodebuild" nil ".log")))
    (let ((cmd (concat "time ( /usr/bin/xcodebuild -project AIR.cocoatouch.xcodeproj " archs-arg cfg-arg tgt-arg
                       " " cmds
                       " | tee " tmp-file
                       " | grep --before-context=5 ':' "
                       "&& tail -5 " tmp-file " )")))
...
(defun compile-including-xcode ()
  ...
  (let* ((suffix ".xcodeproj")
         ...
         (xcode-invoke "/usr/bin/xcodebuild")
