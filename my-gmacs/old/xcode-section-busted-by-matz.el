...
;; or, for AOT, in  code/products/AIR

;empty string will work if only one xcodeprof file in dir..
(setq xcode-project "AIR.cocoatouch.xcodeproj")

(defun xcodebuild-project-specifier-arg ()
  "project specifier if you want one."
  (interactive)
  "-project")

(defun xcodebuild-command ()
  "cooks up the xcodebuild command"
  (interactive)
  "/Developer/usr/bin/xcodebuild")

;  (concat "/Developer/usr/bin/xcodebuild" " " (xcodebuild-project-specifier-arg) " " (xcodebuild-project-specifier-arg)))

(defun xcodebuild-list-command ()
  "cooks up the xcodebuild list command"
  (concat (xcodebuild-command) " -list"))
  
(defun xcodebuild-list-shell ()
  "Prints xcodebuild help and target/configuration list to a shell buffer."
  (interactive)
  (shell-command (concat (xcodebuild-command) " -help")
  (shell-command (concat (xcodebuild-command) " -list"))))

(defun xcodebuild-list ()
  "Returns Xcode target/configuration lines for project in current directory."
  (interactive)
  (process-lines (xcodebuild-command) "-help"))
  
;  (process-lines "/Developer/usr/bin/xcodebuild" "-project" "AIR.cocoatouch.xcodeproj" "-list"))


;  (process-lines (xcodebuild-command) (xcodebuild-project-specifier-arg) xcode-project "-list"))
;   (message (concat (xcodebuild-command) (xcodebuild-project-specifier-arg) xcode-project "-list")))
