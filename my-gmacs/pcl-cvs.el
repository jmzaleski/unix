...
; matz hack to make ediff-revisions go to HEAD by default instead of BASE
...
Returns a list of FIS that should be `cvs remove'd."
  (let* ((files (cvs-mode-marked filter cmd :file t :read-only t))
    ...
    (when (and (not silent) (equal cvs-confirm-removals 'list))
      (save-excursion
	(pop-to-buffer (cvs-temp-buffer cmd 'normal))
	(dolist (fi fis)
	...
	(with-current-buffer buffer
	  (let ((cvs-buf-was-ro buffer-read-only))
	    (ignore-errors
	      ...
	      ;; VC.  Maybe it'd better to reset VC explicitly ?
